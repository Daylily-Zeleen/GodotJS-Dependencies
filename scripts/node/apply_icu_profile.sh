...
# an exact, fail-closed patch: a changed upstream gyp layout must not silently
...
if [ "$delete_tmp_count" -ne 1 ]; then
  echo "Expected exactly one ICU --delete-tmp action, found $delete_tmp_count" >&2
  exit 1
fi
sed -i.bak "s/'--delete-tmp',/ /" "$GYP"
rm -f "$GYP.bak"
